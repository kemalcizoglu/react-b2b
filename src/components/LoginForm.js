...
import { saveToken, getToken } from '../helpers/useToken'
import { saveName } from '../helpers/useUserName'
import { setToken, setUserName } from './../store/site'
import { useNavigate } from "react-router-dom";


const fakeAuth = () =>
    new Promise((resolve) => {
        setTimeout(() => resolve('2342f2f1d131rf12'), 100);
    });
...
const LoginForm = () => {
    ...
    const Login = (e) => {
        ...
        if (data.users[0].email === email && data.users[0].password === password) {
            setIsLogged(true);
            dispatch(setUserName(data.users[0].name + ' ' + data.users[0].lastName));
            saveName(data.users[0].name + ' ' + data.users[0].lastName)
            return true;
        } else if (data.users[1].email === email && data.users[1].password === password) {
            setIsLogged(true);
            dispatch(setUserName(data.users[0].name + ' ' + data.users[0].lastName));
            saveName(data.users[0].name + ' ' + data.users[0].lastName)
            return;
    ...
    return (
        ...
        <Container className="d-flex justify-content-center align-items-center">
            <Row className="mt-5">
                <Col>
                    <h1 className="mb-5">B2B Giriş Formu</h1>
                    <Form>
